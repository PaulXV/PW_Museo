...
public static class ShowsEndpoints
{
    public static void MapShowsEndpoints(this IEndpointRouteBuilder routes)
    {
        ...
        group.MapGet("/", async (IShowsDA repo) => Results.Ok(await repo.GetAllAsync()));
        group.MapGet("/{id:guid}", async (Guid id, IShowsDA repo) =>
        {
            var item = await repo.GetByIdAsync(id);
            return item is not null ? Results.Ok(item) : Results.NotFound();
        });
        group.MapPost("/", async (Show item, IShowsDA repo) =>
        {
            item.Id = Guid.NewGuid();
            await repo.CreateAsync(item);
            return Results.Created($"/api/shows/{item.Id}", item);
        });
        group.MapPut("/{id:guid}", async (Guid id, Show item, IShowsDA repo) =>
        {
        ...
        group.MapDelete("/{id:guid}", async (Guid id, IShowsDA repo) =>
        {
            if (await repo.GetByIdAsync(id) is null) return Results.NotFound();
